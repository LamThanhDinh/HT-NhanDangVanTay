...
if __name__ == '__main__':
    ...
    print(f"\n✅ Đã xử lý {len(images)} ảnh!")
    print(f"📁 Kết quả được lưu tại: {output_dir}")
    print(f"\n💡 Mẹo: Để xem ảnh kết quả với matplotlib (có toolbar zoom/pan/save):")
    print(f"   import matplotlib.pyplot as plt")
    print(f"   result = cv.imread('{output_dir}0.png')")
    print(f"   plt.figure(figsize=(16, 8))")
    print(f"   plt.imshow(cv.cvtColor(result, cv.COLOR_BGR2RGB))  # Chuyển BGR->RGB")
    print(f"   plt.title('Pipeline kết quả - 8 bước')")
